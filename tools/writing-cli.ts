  ...
  try {
    switch (command) {
      ...
      case "graph": {
        ...
        const schemaState = resolveBibleSchema(project);
        const outputPath = buildMermaidGraphs(project, report.chapters, schemaState.schema.categories);
        logLine(`Graph output: ${outputPath}`);
        return;
      }
      default:
...
function buildMermaidGraphs(project: ProjectContext, entries: ChapterEntry[], categories: BibleCategory[]): string {
  const outputDir = path.join(project.distDir, "graphs");
  fs.mkdirSync(outputDir, { recursive: true });

  const generatedAt = new Date().toISOString();
  const lines: string[] = [];
  lines.push(`# Graphs: ${project.meta.title || project.id}`);
  lines.push("");
  lines.push(`Generated: ${generatedAt}`);
  lines.push("");

  lines.push("## File To Entity Graph");
  lines.push("");
  lines.push("```mermaid");
  lines.push(buildFileToEntityMermaid(entries, categories));
  lines.push("```");
  lines.push("");

  lines.push("## Entity Co-occurrence Graph");
  lines.push("");
  lines.push("```mermaid");
  lines.push(buildEntityCooccurrenceMermaid(entries, categories));
  lines.push("```");
  lines.push("");

  lines.push("## Chapter Sequence Graph");
  lines.push("");
  lines.push("```mermaid");
  lines.push(buildChapterSequenceMermaid(entries, categories));
  lines.push("```");
  lines.push("");

  const outputPath = path.join(outputDir, `${project.id}.md`);
  fs.writeFileSync(outputPath, lines.join("\n"), "utf8");
  return outputPath;
}

function buildFileToEntityMermaid(entries: ChapterEntry[], categories: BibleCategory[]): string {
  const lines: string[] = ["graph LR"];
  const entryIds = new Map<ChapterEntry, string>();
  const categoryEntityIds = new Map<string, Set<string>>();
  const edges: Array<{ from: string; to: string; label: string }> = [];

  for (const category of categories) {
    categoryEntityIds.set(category.key, new Set<string>());
  }

  entries.forEach((entry, index) => {
    entryIds.set(entry, `F${index + 1}`);
  });

  for (const entry of entries) {
    for (const category of categories) {
      const refs = getEntryCategoryRefs(entry, category.key);
      for (const ref of refs) {
        categoryEntityIds.get(category.key)?.add(ref);
        edges.push({
          from: entryIds.get(entry) || "F0",
          to: `E_${toNodeToken(ref)}`,
          label: category.key
        });
      }
    }
  ...
  lines.push(`  subgraph FILES["Files"]`);
  for (const entry of entries) {
    const id = entryIds.get(entry) || "F0";
    const label = `${entry.order ?? "?"}. ${entry.title}`;
    lines.push(`    ${id}["${escapeMermaidLabel(label)}"]`);
  }
  lines.push("  end");

  for (const category of categories) {
    const refs = [...(categoryEntityIds.get(category.key) || new Set<string>())].sort();
    lines.push(`  subgraph CAT_${toNodeToken(category.key)}["${escapeMermaidLabel(category.key)}"]`);
    if (refs.length === 0) {
      lines.push(`    CAT_EMPTY_${toNodeToken(category.key)}["(no refs)"]`);
    } else {
      for (const ref of refs) {
        lines.push(`    E_${toNodeToken(ref)}["${escapeMermaidLabel(ref)}"]`);
      }
    }
    lines.push("  end");
  }
  ...
  if (edges.length === 0) {
    lines.push(`  EMPTY["No category references found"]`);
  } else {
...
function buildEntityCooccurrenceMermaid(entries: ChapterEntry[], categories: BibleCategory[]): string {
  const lines: string[] = ["graph LR"];
  ...
  for (const entry of entries) {
    ...
    for (const category of categories) {
      for (const id of getEntryCategoryRefs(entry, category.key)) {
        refs.add(id);
      }
    }
  ...
  if (entitySet.size === 0) {
    lines.push(`  EMPTY["No entity references found"]`);
    return lines.join("\n");
  }

  for (const id of [...entitySet].sort()) {
    lines.push(`  E_${toNodeToken(id)}["${escapeMermaidLabel(id)}"]`);
  }

  if (cooccurrence.size === 0) {
    lines.push(`  SOLO["No co-occurrences detected"]`);
    return lines.join("\n");
  }

  for (const [pair, count] of [...cooccurrence.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
    const [left, right] = pair.split("|||");
    lines.push(`  E_${toNodeToken(left)} ---|"${count}"| E_${toNodeToken(right)}`);
  }

  return lines.join("\n");
}

function buildChapterSequenceMermaid(entries: ChapterEntry[], categories: BibleCategory[]): string {
  const lines: string[] = ["graph TD"];
  const sections = buildChapterSections(entries);

  if (sections.length === 0) {
    lines.push(`  EMPTY["No chapter sections found"]`);
    return lines.join("\n");
  }
  ...
  for (let index = 0; index < sections.length; index += 1) {
    ...
    for (const category of categories) {
      ...
      for (const entry of section.entries) {
        count += getEntryCategoryRefs(entry, category.key).length;
      }
      stats.push(`${category.key}:${count}`);
    }

    const nodeId = `CH_${index + 1}`;
    const heading = getSectionHeading(section, index, sections.length);
    const statsLabel = stats.join(" | ");
    const label = statsLabel ? `${heading}<br/>${statsLabel}` : heading;
    lines.push(`  ${nodeId}["${escapeMermaidLabel(label)}"]`);
  }

  for (let i = 0; i < sections.length - 1; i += 1) {
    lines.push(`  CH_${i + 1} --> CH_${i + 2}`);
  }

  return lines.join("\n");
}

function getEntryCategoryRefs(entry: ChapterEntry, categoryKey: string): string[] {
  const raw = entry.metadata[categoryKey];
  if (!Array.isArray(raw)) {
    return [];
  }
...
function toNodeToken(value: string): string {
